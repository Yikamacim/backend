...
export class TopBusinessesParams implements IParams {
  private constructor(public readonly neighborhoodId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<TopBusinessesParams | null> {
    ...
    if (!TopBusinessesParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: TopBusinessesParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is TopBusinessesParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as TopBusinessesParams;
