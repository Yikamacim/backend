...
export class BusinessesParams implements IParams {
  private constructor(public readonly businessId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<BusinessesParams | null> {
    ...
    if (!BusinessesParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: BusinessesParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is BusinessesParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as BusinessesParams;
