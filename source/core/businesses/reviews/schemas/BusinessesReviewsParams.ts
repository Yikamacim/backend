...
export class BusinessesReviewsParams implements IParams {
  private constructor(public readonly businessId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<BusinessesReviewsParams | null> {
    ...
    if (!BusinessesReviewsParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: BusinessesReviewsParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is BusinessesReviewsParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as BusinessesReviewsParams;
