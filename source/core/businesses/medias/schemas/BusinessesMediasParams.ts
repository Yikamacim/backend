...
export class BusinessesMediasParams implements IParams {
  private constructor(public readonly businessId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<BusinessesMediasParams | null> {
    ...
    if (!BusinessesMediasParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: BusinessesMediasParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is BusinessesMediasParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as BusinessesMediasParams;
