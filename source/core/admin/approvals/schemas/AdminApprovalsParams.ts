...
export class AdminApprovalsParams implements IParams {
  private constructor(public readonly businessId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<AdminApprovalsParams | null> {
    ...
    if (!AdminApprovalsParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: AdminApprovalsParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is AdminApprovalsParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as AdminApprovalsParams;
