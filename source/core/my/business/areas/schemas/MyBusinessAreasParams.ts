...
export class MyBusinessAreasParams implements IParams {
  private constructor(public readonly neighborhoodId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<MyBusinessAreasParams | null> {
    ...
    if (!MyBusinessAreasParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: MyBusinessAreasParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is MyBusinessAreasParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as MyBusinessAreasParams;
