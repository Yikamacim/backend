...
export class MyOrdersCancelParams implements IParams {
  private constructor(public readonly orderId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<MyOrdersCancelParams | null> {
    ...
    if (!MyOrdersCancelParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: MyOrdersCancelParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is MyOrdersCancelParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as MyOrdersCancelParams;
