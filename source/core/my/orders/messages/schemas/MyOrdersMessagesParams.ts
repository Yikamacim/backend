...
export class MyOrdersMessagesParams implements IParams {
  private constructor(public readonly orderId: string) {}

  public static parse(req: ExpressRequest): ParserResponse<MyOrdersMessagesParams | null> {
    ...
    if (!MyOrdersMessagesParams.isBlueprint(protovalidData)) {
      return ResponseUtil.parserResponse(
        [new ClientError(ClientErrorCode.INVALID_PARAMETER)],
        null,
      );
    }
    const blueprintData: MyOrdersMessagesParams = protovalidData;
    // >----------< PHYSICAL VALIDATION >----------<
  ...
  private static isBlueprint(data: unknown): data is MyOrdersMessagesParams {
    if (typeof data !== "object" || data === null) {
      return false;
    }
    const blueprint = data as MyOrdersMessagesParams;
